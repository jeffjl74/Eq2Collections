 ...
 * Changelog:
 * .4
 * - Fix for DDPF_RGB when dwPitchOrLinearSize = 0
 * .3
...
namespace Eq2Collections
{
    public class DDSImage {
		...
		public DDSImage(byte[] rawdata) {
			using (MemoryStream ms = new MemoryStream(rawdata)) {
				using (BinaryReader r = new BinaryReader(ms)) {
					...
					int byteLen = header.dwPitchOrLinearSize;
                    if ((header.ddspf.dwFlags & DDPF_RGB) != 0)
						byteLen = header.dwHeight * header.dwWidth * (header.ddspf.dwRGBBitCount / 8); //8 bits per byte
                    bdata = r.ReadBytes(byteLen);
        ...
        private void DecompressBlockDXT3(int x, int y, byte[] blockStorage, Bitmap image)
        {
            //build an alpha lookup table
            byte[] alpha = new byte[16];
            //BC2
            for(int i=0; i<8; i++)
            {
                uint bits = (uint)(blockStorage[i] & 0xf);
                alpha[2*i] = (byte)((bits * 255) / 15);

                bits = (uint)(blockStorage[i] >> 4);
                alpha[(2*i)+1] = (byte)((bits * 255) /15);
            }
            ...
            for (int j = 0; j < 4; j++)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    if (color0 > color1)
                    {
                        ...
                        {
                            case 0:
                                finalColor = Color.FromArgb(alphaval, r0, g0, b0);
                                break;
                            case 1:
                                finalColor = Color.FromArgb(alphaval, r1, g1, b1);
                                break;
                            case 2:
                                finalColor = Color.FromArgb(alphaval, (2 * r0 + r1) / 3, (2 * g0 + g1) / 3, (2 * b0 + b1) / 3);
                                break;
                            case 3:
                                finalColor = Color.FromArgb(alphaval, (r0 + 2 * r1) / 3, (g0 + 2 * g1) / 3, (b0 + 2 * b1) / 3);
                                break;
                        }
                    }
                    else
                    {
                        switch (positionCode)
                        {
                            ...
                            case 1:
                                ...
                                break;
                            case 2:
                                finalColor = Color.FromArgb(alphaval, (r0 + r1) / 2, (g0 + g1) / 2, (b0 + b1) / 2);
                                break;
                            case 3:
                                finalColor = Color.FromArgb(alphaval, 0, 0, 0);
                                break;
                        }
                    }
			...
			for (int j = 0; j < 4; j++) {
				for (int i = 0; i < 4; i++) {
					...
					if (alphaCodeIndex <= 12) {
						...
						alphaCode = (int)((ushort)(alphaCode2 >> 15) | (uint)((alphaCode1 << 1) & 0x06));
					} else {
		...
		private Bitmap readLinearImage(byte[] data, int w, int h) {
			Bitmap res = new Bitmap(w, h);
			if (data.Length >= w * h * sizeof(Int32))
			{
				using (MemoryStream ms = new MemoryStream(data))
				{
					using (BinaryReader r = new BinaryReader(ms))
